...
export default function Menu() {
  ...
  return (
    <>
      <div
        className="absolute mobile:bg-transparent  bg-black/5 inset-0  w-full h-full z-20"
        onClick={onClose}
      />
      <div
        className={`absolute flex flex-col gap-1 z-50 text-[--color] bg-[hsl(var(--background))] dark:bg-[rgb(27,28,29)] border-[hsl(var(--border-color))] shadow-md border rounded-[15px] p-2 w-[240px] ${
          mobile && "rounded-bl-none rounded-br-none pt-5 px-4 bdy-hn"
        }`}
        style={
          mobile
      ...
      >
        {menuList.map((item: any, index: number) => (
            item.type !=="hide" &&
            <button
                className={`${item.type==="danger" ? "text-red-400" :"text-[--color]"}  flex w-full justify-between text-[15px] items-center gap-2 rounded-lg px-3 py-2 dark:hover:bg-white/5 hover:bg-[hsl(var(--accent))]`}
            ...
            >
                <span>{item.name}</span>
                <IconRenderer key={index} iconName={item.id} />
