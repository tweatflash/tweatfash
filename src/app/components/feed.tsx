...
export default function Feed({dave}: daveA) {
    ...
    const [liked,setLiked]=useState(userObj?.user && userObj.user.likedPosts.some((item:any) => item._id == dave._id) ? true :false)
    const [saved,setSaved]=useState(userObj?.user && userObj.user.flags.some((item:any) => item == dave._id) ? true :false)
    ...
    const saveAPost=async (postId:string)=>{
        if (saved){
            setSaved(false)
            userObj.user.flags= userObj.user.flags.filter((item :string)=> item !==dave._id)
        }else{
            setSaved(true)
